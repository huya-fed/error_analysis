...
Class.pt.creat = function(){
	...
	switch(config.type){
		...
		break;
		case 2:
			var content = config.content = conType ? config.content : [config.content||'http://www.huya.com', 'auto'];
	...
	if(config.fixed){
		win.on('resize.'+that.index, function(){
			that.offset();
...
Class.pt.move = function(){
	...
	_DOC.off('mousemove.'+that.index).off('mouseup.'+that.index).on('mousemove.'+that.index, function(e){
	...
	}).on('mouseup.'+that.index, function(e){
		if(dict.moveStart){
